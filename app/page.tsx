...
const roles = [
  ...
  { period: "2010 — 2021", company: "CDAC · Government of India", role: "SDE 2 → SDE 3 → Architect → Engineering Manager", summary: "Progressed from individual contributor to technical architect and engineering manager at India’s national government R&D institute.", details: ["Led 12 engineers across IoT and AR, owning hiring, performance management, PIPs, and team development.", "Owned a ₹300 Cr+ portfolio across five government-scale initiatives, including a ₹250 Cr LMS proposal developed with NASSCOM.", "Delivered 10+ public products: COVID Mitra across four states, a unified LMS for 50+ institutions, and Smart City dashboards in three cities.", "Architected a 45 GB/day Smart Water pipeline spanning 8,000 sensors across 40 locations.", "Independently built the official Namma Metro Android app, reaching 1.2M+ downloads.", "Grew an Android/AR team from two to seven and delivered the Smart Post Kiosk and EPFO identity-verification backend."], wins: ["12-engineer organisation", "₹300 Cr+ portfolio", "10+ public products", "1.2M+ app downloads", "8,000 IoT sensors", "4-state COVID deployment"], tech: ["Java", "Android SDK", "Node.js", "Angular", "React", "IoT Data Pipelines"] },
];
...
export default function Home() {
  ...
  return <main id="top">
    ...
    <section className="section shell" id="work"><SectionTitle number="01" label="Career runtime">Systems that operate at scale.</SectionTitle><div className="timeline">{roles.map((role, index) => <article className="timeline-row" key={role.company}><div className="node">0{index + 1}</div><time>{role.period}</time><div><h3>{role.company}</h3><p className="role-title">{role.role}</p><p>{role.summary}</p>{role.details && <ul className="role-details">{role.details.map(detail => <li key={detail}>{detail}</li>)}</ul>}<ul className="role-wins">{role.wins.map(win => <li key={win}>{win}</li>)}</ul><div className="role-stack"><span>TECH_STACK[]</span><ul>{role.tech.map(technology => <li key={technology}>{technology}</li>)}</ul></div></div></article>)}</div></section>
