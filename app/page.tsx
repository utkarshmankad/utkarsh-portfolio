...
export default function Home() {
  ...
  return <main id="top">
    <nav className="nav shell" aria-label="Primary navigation"><a className="brand" href="#top"><span>U</span>M_</a><div className="nav-links"><a href="#work">01.Work</a><a href="#isha">02.Isha</a><a href="#projects">03.Builds</a><a href="#open-source">04.OSS</a><a href="#contact">05.Contact</a></div><button className="theme-button" onClick={() => setDark(!dark)} aria-label={`Switch to ${dark ? "light" : "dark"} theme`}><span aria-hidden>{dark ? "☼" : "◐"}</span>{dark ? "Light" : "Dark"}</button></nav>
    ...
    <section className="isha-section" id="isha"><div className="shell"><SectionTitle number="02" label="Flagship AI build">Isha: answers grounded in evidence.</SectionTitle><div className="isha-grid"><div className="isha-copy"><p className="product-code">ISHA / INDIGO SMART HELPDESK ASSISTANT</p><h3>A production-minded RAG assistant for airline passenger queries.</h3><p>Isha routes passenger questions to the right policy corpus, fuses lexical and vector retrieval, checks confidence, and generates cited answers through a LangGraph workflow.</p><a className="tech-link" href="https://github.com/utkarshmankad/isha-indigo-rag" target="_blank" rel="noreferrer">View source on GitHub <span>↗</span></a></div><div className="architecture" aria-label="Isha retrieval architecture"><div><span>01</span>Query router</div><i>→</i><div><span>02</span>BM25 + Qdrant</div><i>→</i><div><span>03</span>Confidence gate</div><i>→</i><div><span>04</span>Cited response</div></div><div className="isha-features"><article><b>Hybrid retrieval</b><p>BM25 and vector search fused with reciprocal rank fusion.</p></article><article><b>Trust layer</b><p>Confidence refusal, citations, RAGAS evaluation, and observability.</p></article><article><b>Intelligence</b><p>HyDE, conversation memory, Hindi support, and tool routing.</p></article><article><b>Product surface</b><p>FastAPI wrapper, embeddable widget, multi-tenancy, and admin ingestion.</p></article></div></div></div></section>

    <section className="section shell" id="products"><SectionTitle number="03" label="Products shipped">From cloud platforms to city infrastructure.</SectionTitle><div className="product-grid">{products.map((product, index) => <a href={product.href} target="_blank" rel="noreferrer" key={product.name}><small>0{index + 1} / {product.meta}</small><h3>{product.name}</h3><p>{product.description}</p><span>Open project ↗</span></a>)}</div></section>

    <section className="section shell" id="projects"><SectionTitle number="04" label="Independent builds">Products, not placeholders.</SectionTitle><div className="project-grid">{projects.map(project => <a href={project.href} target="_blank" rel="noreferrer" key={project.name}><span className="project-tech">{project.tech}</span><h3>{project.name}</h3><p>{project.description}</p><b>Explore build ↗</b></a>)}</div></section>

    <section className="oss-section" id="open-source"><div className="shell"><SectionTitle number="05" label="Open source signal">Contributing beyond my own repositories.</SectionTitle><div className="oss-grid">{contributions.map(item => <a href={item.href} target="_blank" rel="noreferrer" key={`${item.project}-${item.code}`}><div><span>{item.project}</span><i className={item.status.toLowerCase()}>{item.status}</i></div><small>{item.code}</small><h3>{item.title}</h3><p>{item.detail}</p><b>Inspect contribution ↗</b></a>)}</div></div></section>

    <section className="section shell split-section"><div><SectionTitle number="06" label="Leadership protocol">Clarity over noise. Outcomes over activity.</SectionTitle><p>Durable systems and durable teams are built the same way: with clear interfaces, honest feedback, calm ownership, and room to grow.</p><blockquote>Turn ambiguity into direction—and leave every system stronger than you found it.</blockquote></div><div id="skills"><p className="skill-label">CORE_CAPABILITIES[]</p><ul className="skill-list">{capabilities.map((capability, index) => <li key={capability}><span>{String(index).padStart(2, "0")}</span>{capability}</li>)}</ul><p className="stack">Java / Python / Scala / Spark / TypeScript / React / Kafka / PostgreSQL / Kubernetes / AWS / GCP / OCI / RAG / LangGraph</p></div></section>
