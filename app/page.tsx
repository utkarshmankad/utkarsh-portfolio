...
export default function Home() {
  return <main id="top">
    ...
    <header className="hero shell" id="content" tabIndex={-1}>
      ...
      <div className="hero-bottom"><p>{siteConfig.description}</p><div className="hero-actions" aria-label="Portfolio actions"><a className="primary-action" href="#work"><span aria-hidden>$</span> View selected work</a><a href="/utkarsh-mankad-resume.pdf" download>Download résumé ↓</a><a href="#contact">Contact me ↘</a></div></div>
    ...
    <footer id="contact"><div className="shell"><p className="footer-code">READY_FOR_NEXT_CHALLENGE = TRUE</p><h2>Let’s engineer<br /><em>what’s next.</em></h2><div className="contact-actions"><a className="contact-link" href={`mailto:${siteConfig.email}`}>Email Utkarsh <span>↗</span></a><a className="resume-link" href="/utkarsh-mankad-resume.pdf" download>Download résumé ↓</a></div><div className="footer-bottom"><span>© {new Date().getFullYear()} Utkarsh Mankad</span><div><a href={siteConfig.github} target="_blank" rel="noreferrer">GitHub ↗</a><a href={siteConfig.linkedin} target="_blank" rel="noreferrer">LinkedIn ↗</a></div><a href="#top">Back to top ↑</a></div></div></footer>
  </main>;
}
