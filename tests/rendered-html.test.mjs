...
try{
 ...
 assert.match(html,/Utkarsh Mankad/);assert.match(html,/Engineering/);assert.match(html,/Isha/);assert.match(html,/AutoMQ/);assert.match(html,/RisingWave/);assert.match(html,/Oracle 23ai/);assert.match(html,/Debezium/);assert.match(html,/Android SDK/);assert.match(html,/leading 12 engineers across IoT and AR/);assert.match(html,/public systems spanning smart water infrastructure/);assert.match(html,/Executive stakeholder management/);assert.match(html,/Budget &amp; portfolio ownership/);assert.match(html,/Cross-functional programme delivery/);assert.match(html,/Automated Canary Deployments/);assert.match(html,/Architecture of Spark Connectors/);assert.match(html,/GitHub/);assert.match(html,/LinkedIn/);assert.doesNotMatch(html,/CORE_CAPABILITIES|Chandy–Misra–Haas|Inverted Index Search|Suzuki–Kasami Broadcast|codex-preview|SkeletonPreview|UTKARSH\.JPG/);
